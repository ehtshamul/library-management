...
export default function ReviewsDisplay({ BookID }) {
  ...
  useEffect(() => {
    if (BookID) {
      dispatch(showReview(BookID)); // ✅ pass BookID to API
    }
  }, [dispatch, BookID]);
  ...
  if (error) {
    return (
      <div className="bg-white rounded-lg border border-red-200 p-8 text-center">
        <p className="text-red-500">Failed to load reviews: {typeof error === 'object' ? (error?.message ?? JSON.stringify(error)) : error}</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {reviews.map((review, index) => (
        ...
        >
          ...
          {auth?.user && (() => {
            // resolve owner id from possible shapes
            const ownerId = review.userID?._id || review.userID?.id || review.userID || null;
            const currentUserId = auth.user?.id || auth.user?._id || null;
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        await dispatch(reviewdelete(review._id || review.id)).unwrap();
                      } catch (err) {
                        console.error("Failed to delete review", err);
                        // optionally set a toast or state here
                      } finally {
                        setDeletingId(null);
                      }
                    }}
